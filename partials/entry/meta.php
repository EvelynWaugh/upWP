...
<?php if (get_post_type() === 'events') : ?>
	<ul class="meta obem-<?php echo $meta_class; ?> clr" aria-label="<?php esc_attr_e('Post details:', 'oceanwp'); ?>">
		...
		$cats = wp_get_post_terms(get_the_ID(), 'events_category');
		...
		?>
			...
			<?php if ('categories' === $section) { ?>
				<li class="meta-cat"><span class="screen-reader-text"><?php esc_html_e('Post category:', 'oceanwp'); ?></span><?php oceanwp_icon('category'); ?>
					<?php foreach ($cats as $cat) : ?>

						<a href="<?php echo get_term_link($cat->term_id, 'events_category'); ?>"><?php echo $cat->name; ?></a>
					<?php endforeach; ?>
				</li>
			<?php } ?>
